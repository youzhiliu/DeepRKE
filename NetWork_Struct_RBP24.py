...
struct_word2vec_model='./RNAShapes2Vec_GraphProt'
seq_word2vec_model='./RNA2Vec_GraphProt'

class BCL_Network_GraphProt(nn.Module):
		def __init__(self):
				super(BCL_Network_GraphProt, self).__init__()
		...
		def forward(self, seq, struct):
				...
				# input = torch.cat((seq, struct), 1)
				# cnn_output_1 = self.cnn(input)
				seq_out = self.cnn(seq)
				struct_out = self.cnn(struct)
				# 序列和结构横向拼接
				cnn_output_1 = torch.cat((seq_out, struct_out), 1)

				cnn_output = self.cnn_1(cnn_output_1)
